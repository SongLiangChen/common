...
//添加sku
func AddSku(orgId int, mchId, productId int64, specs map[string]string, sortOrder int, price string, stock int) (*Sku, *base_server_sdk.Error) {
	if orgId <= 0 || mchId <= 0 || productId <= 0 || len(specs) == 0 || sortOrder <= 0 || price == "" || stock <= 0 {
		return nil, base_server_sdk.ErrInvalidParams
	}
	...
	params["price"] = price
	params["stock"] = strconv.Itoa(stock)
...
// 操作库存
func OperateStock(orgId int, mchId, productId, skuId int64, qty int, opType model.StockOpType) *base_server_sdk.Error {
	if orgId <= 0 || productId <= 0 || skuId <= 0 || qty <= 0 || opType <= 0 {
